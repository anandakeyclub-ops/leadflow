"""
scrape_lake_permits.py
================================
Lake County building permits via Accela Citizen Access.
Portal: https://aca-prod.accela.com/LAKE/Cap/CapHome.aspx?module=Building

Strategy: Search per permit type (19 types) with date-split to bypass 59-row cap.
Confirmed working approach from permit_bot download_lake_weekly.py.

Usage:
  python -m app.workers.scrape_lake_permits --days-back 30 --visible
  python -m app.workers.scrape_lake_permits --days-back 7
"""
...
# ---------------------------------------------------------------------------
COUNTY_NAME = "Lake"
SOURCE_NAME = "lake_accela"
MODULE_URL  = "https://aca-prod.accela.com/LAKE/Cap/CapHome.aspx?module=Building&TabName=Building"
...
RAW_DIR  = BASE_DIR / "data" / "raw" / "lake" / "permits"
...
def row_to_permit(row: dict) -> Optional[PermitRecord]:
    ...
    # Reject garbage rows — valid Lake permit numbers look like:
...
def scrape_lake(start: date, end: date, visible: bool = False) -> List[PermitRecord]:
    """Scrape all Lake permits for the date range."""
    ...
    print(f"\n[Lake] Scraping {start_str} → {end_str}")
...
def main():
    parser = argparse.ArgumentParser(description="Lake County permit scraper")
    ...
    records = scrape_lake(start, end, visible=args.visible)

    if records:
        snap = RAW_DIR / f"lake_permits_{nowstamp()}.json"
        snap.write_text(
