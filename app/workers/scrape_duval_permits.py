...
================================
Sarasota County building permits via Accela Citizen Access.
Portal: https://aca-prod.accela.com/SARASOTACO/Cap/CapHome.aspx?module=Building

Strategy: Search per permit type (19 types) with date-split to bypass 59-row cap.
Confirmed working approach from permit_bot download_sarasota_weekly.py.

Usage:
  python -m app.workers.scrape_sarasota_permits --days-back 30 --visible
  python -m app.workers.scrape_sarasota_permits --days-back 7
"""
...
MODULE_URL  = "https://aca-prod.accela.com/SARASOTACO/Cap/CapHome.aspx?module=Building"

# Confirmed permit types from live portal 2026-04-10
SEARCH_TYPES = [
    "Commercial Roof Trade Permit",
    "Residential Roof Trade Permit",
    "Commercial Mechanical Trade Permit",
    "Residential Mechanical Trade Permit",
    "Residential Mechanical Trade Permit - Exempt",
    "Commercial Electrical Trade Permit",
    "Residential Electrical Trade Permit",
    "Commercial Plumbing Trade Permit",
    "Residential Plumbing Trade Permit",
    "Commercial Building Alterations (Renovations)",
    "Commercial Building Trade Permit",
    "Commercial Demolition Permit",
    "Commercial New Construction and Additions",
    "Residential Building Alterations (Renovations)",
    "Residential Building Trade Permit",
    "Residential Demolition Permit",
    "Residential New Construction and Additions",
    "Commercial Alteration Building Permit Partial CO",
    "Commercial New Building Permit Partial CO",
]
...
LOGIN_URL = "https://aca-prod.accela.com/SARASOTACO/Account/LogIn.aspx"


def login(driver) -> bool:
    """Log in to Sarasota Accela portal — form is inside an iframe."""
    if not SARASOTA_USER or not SARASOTA_PASS:
        print("  No Sarasota Accela credentials — set SARASOTA_ACCELA_USER/PASS in .env")
        return False

    print(f"  Logging in as {SARASOTA_USER}...")
    driver.get("https://aca-prod.accela.com/SARASOTACO/Login.aspx")
    time.sleep(5)

    try:
        # Login form is inside iframe id="LoginFrame"
        iframe = driver.find_element(By.ID, "LoginFrame")
        driver.switch_to.frame(iframe)
        time.sleep(2)
        print("  Switched to LoginFrame iframe")

        # Fields confirmed from MHTML:
        # id="username", id="passwordRequired"
        # button class="ACA_Button p-button p-component"
        user_el = driver.find_element(By.ID, "username")
        user_el.clear()
        user_el.send_keys(SARASOTA_USER)
        time.sleep(0.3)

        pass_el = driver.find_element(By.ID, "passwordRequired")
        pass_el.clear()
        pass_el.send_keys(SARASOTA_PASS)
        time.sleep(0.3)

        # Click Sign In button
        sign_in = driver.find_element(By.XPATH,
            "//button[contains(@class,'ACA_Button')]"
        )
        ...
        if "Login" not in driver.current_url:
            ...
            print(f"  Still on login page — check credentials")
            return False
...
def search_one_type(driver, permit_type: str, start_str: str, end_str: str) -> List[dict]:
    ...
    # If dropdown not found, dump all selects on page to find correct ID
    if not dropdown_found:
        print(f"    Dropdown {TYPE_DROPDOWN_ID!r} not found — dumping all selects:")
        selects = driver.find_elements(By.TAG_NAME, "select")
        for sel in selects:
            sid = sel.get_attribute("id") or ""
            opts = [o.text.strip() for o in sel.find_elements(By.TAG_NAME, "option")]
            if any(k in ' '.join(opts).upper() for k in ['PERMIT', 'ROOF', 'BUILDING', 'TRADE']):
                print(f"    SELECT id={sid!r} options={opts[:10]}")
        return []

    # On first run, always dump all options so we can update SEARCH_TYPES
...
def row_to_permit(row: dict) -> Optional[PermitRecord]:
    ...
    # Reject garbage rows — valid Sarasota permit numbers look like:
...
def scrape_sarasota(start: date, end: date, visible: bool = False) -> List[PermitRecord]:
    """Scrape all Sarasota permits for the date range."""
    ...
    print(f"\n[Sarasota] Scraping {start_str} → {end_str}")
...
def main():
    ...
    records = scrape_sarasota(start, end, visible=args.visible)
