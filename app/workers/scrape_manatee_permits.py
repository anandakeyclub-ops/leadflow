"""
scrape_manatee_permits.py
================================
Manatee County building permits via Accela Citizen Access.
Portal: https://aca-prod.accela.com/MANATEE/Cap/CapHome.aspx?module=Building

Strategy: Search per permit type (19 types) with date-split to bypass 59-row cap.
Confirmed working approach from permit_bot download_manatee_weekly.py.

Usage:
  python -m app.workers.scrape_manatee_permits --days-back 30 --visible
  python -m app.workers.scrape_manatee_permits --days-back 7
"""
...
# ---------------------------------------------------------------------------
COUNTY_NAME = "Manatee"
SOURCE_NAME = "manatee_accela"
MODULE_URL  = "https://aca-prod.accela.com/MANATEE/Cap/CapHome.aspx?module=Building&TabName=Building"

# Confirmed permit types from live portal 2026-04-10
SEARCH_TYPES = [
    "AC Change Out Commercial Express",
    "AC Change Out Residential Express",
    "Mechanical",
    "Commercial Roof Express",
    "Residential Roof Express",
    "Roof Standard",
    "Electrical",
    "Generator",
    "Residential Electrical Express",
    "Plumbing",
    "Residential Plumbing Express",
    "Pool-Spa",
    ...
    "Residential",
    "Residential Door and Window Express",
...
RAW_DIR  = BASE_DIR / "data" / "raw" / "manatee" / "permits"
...
def row_to_permit(row: dict) -> Optional[PermitRecord]:
    ...
    # Reject garbage rows — valid Manatee permit numbers look like:
...
def scrape_manatee(start: date, end: date, visible: bool = False) -> List[PermitRecord]:
    """Scrape all Manatee permits for the date range."""
    ...
    print(f"\n[Manatee] Scraping {start_str} → {end_str}")
...
def main():
    parser = argparse.ArgumentParser(description="Manatee County permit scraper")
    ...
    records = scrape_manatee(start, end, visible=args.visible)

    if records:
        snap = RAW_DIR / f"manatee_permits_{nowstamp()}.json"
        snap.write_text(
