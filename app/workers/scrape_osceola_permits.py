"""
scrape_osceola_permits.py
================================
Osceola County building permits via Accela Citizen Access.
Portal: https://permits.osceola.org/CitizenAccess/Cap/CapHome.aspx?module=Building

Strategy: Search per permit type (19 types) with date-split to bypass 59-row cap.
Confirmed working approach from permit_bot download_osceola_weekly.py.

Usage:
  python -m app.workers.scrape_osceola_permits --days-back 30 --visible
  python -m app.workers.scrape_osceola_permits --days-back 7
"""
...
# ---------------------------------------------------------------------------
COUNTY_NAME = "Osceola"
SOURCE_NAME = "osceola_accela"
MODULE_URL  = "https://permits.osceola.org/CitizenAccess/Cap/CapHome.aspx?module=Building&TabName=Building"
...
SEARCH_TYPES = [
    ...
    "Gas Permit",
    "Pool Master File",
    "Pool Enclosure Master File",
    "Building Permit",
    "Single Family Master File",
    "Duplex Master File",
    "Demolition Permit",
    "Slab Only Permit",
]
...
RAW_DIR  = BASE_DIR / "data" / "raw" / "osceola" / "permits"
...
def row_to_permit(row: dict) -> Optional[PermitRecord]:
    ...
    # Reject garbage rows — valid Osceola permit numbers look like:
...
def scrape_osceola(start: date, end: date, visible: bool = False) -> List[PermitRecord]:
    """Scrape all Osceola permits for the date range."""
    ...
    print(f"\n[Osceola] Scraping {start_str} → {end_str}")
...
def main():
    parser = argparse.ArgumentParser(description="Osceola County permit scraper")
    ...
    records = scrape_osceola(start, end, visible=args.visible)

    if records:
        snap = RAW_DIR / f"osceola_permits_{nowstamp()}.json"
        snap.write_text(
