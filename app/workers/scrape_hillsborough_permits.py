"""
scrape_hillsborough_permits.py
================================
Hillsborough County building permits via Accela Citizen Access.
Portal: https://aca-prod.accela.com/HCFL/Cap/CapHome.aspx?module=Building

Strategy: Search per permit type (19 types) with date-split to bypass 59-row cap.
Confirmed working approach from permit_bot download_hillsborough_weekly.py.

Usage:
  python -m app.workers.scrape_hillsborough_permits --days-back 30 --visible
  python -m app.workers.scrape_hillsborough_permits --days-back 7
"""
...
SOURCE_NAME = "hillsborough_accela"
MODULE_URL  = "https://aca-prod.accela.com/HCFL/Cap/CapHome.aspx?module=Building&TabName=Building"
...
RAW_DIR  = BASE_DIR / "data" / "raw" / "hillsborough" / "permits"
...
def row_to_permit(row: dict) -> Optional[PermitRecord]:
    ...
    # Reject garbage rows — valid Hillsborough permit numbers look like:
...
def scrape_hillsborough(start: date, end: date, visible: bool = False) -> List[PermitRecord]:
    """Scrape all Hillsborough permits for the date range."""
    ...
    print(f"\n[Hillsborough] Scraping {start_str} → {end_str}")
...
def main():
    parser = argparse.ArgumentParser(description="Hillsborough County permit scraper")
    ...
    records = scrape_hillsborough(start, end, visible=args.visible)

    if records:
        snap = RAW_DIR / f"hillsborough_permits_{nowstamp()}.json"
        snap.write_text(
