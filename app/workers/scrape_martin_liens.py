...
Usage:
  python -m app.workers.scrape_martin_liens --days-back 180 --no-pdf
  python -m app.workers.scrape_martin_liens --days-back 14
"""
...
# ---------------------------------------------------------------------------
COUNTY_NAME = "Martin"
SOURCE_NAME = "martin_liens"

HOME_URL = "https://or.martinclerk.com/LandMarkWeb/Home/Index"
...
RAW_DIR  = BASE_DIR / "data" / "raw" / "martin" / "liens"
...
def download_pdf(driver, rec: dict) -> Optional[str]:
    ...
    pdf_path = PDF_DIR / f"martin_{safe}.pdf"
...
def load_and_accept(driver) -> bool:
    ...
    for xpath in [
        ...
        # Try direct JS navigation
        try:
            driver.execute_script("LaunchDisclaimer('searchCriteriaDocuments');")
            time.sleep(3)
            print(f"  Navigated via JS — {driver.current_url}")
        except Exception:
            pass

    # CRITICAL: Click Document Type Search tab — disclaimer lands on Name Search tab
    # Confirmed tab id from inspector: searchCriteriaDocuments-tab
    time.sleep(1)
    try:
        ...
        try:
            tab = driver.find_element(By.XPATH,
                "//a[contains(text(),'Document Type Search') or contains(text(),'Document Type')]")
            driver.execute_script("arguments[0].click();", tab)
            time.sleep(2)
            print(f"  Clicked Document Type tab via text")
    ...
    try:
        with conn.cursor() as cur:
            ...
            for rec in records:
                ...
                n_hash = f"martin::{rec['instrument_number']}::{(rec['debtor_name'] or '')[:40]}"
...
def main():
    ...
    # Save snapshot
    if records:
        snap = RAW_DIR / f"martin_liens_{nowstamp()}.json"
