"""
scrape_orange_liens.py
====================
Orange County IRS/state tax lien scraper.
Portal: https://or.occompt.com/recorder/ (Acclaim/Harris)
...
Usage:
  python -m app.workers.scrape_orange_liens --days-back 180
  python -m app.workers.scrape_orange_liens --days-back 7 --no-db --visible
"""
...
COUNTY_NAME  = "Orange"
SOURCE_NAME  = "orange_liens"
HASH_PREFIX  = "orange"
HOME_URL = "https://or.occompt.com/recorder/web/"
SEARCH_URL   = "https://or.occompt.com/recorder/web/search/SearchTypeDocumentType"

BASE_DIR = Path(__file__).resolve().parents[2]
RAW_DIR  = BASE_DIR / "data" / "raw" / "orange" / "liens"
        ...
        for pattern in search_dirs:
            for f in glob.glob(pattern):
                if os.path.getmtime(f) > download_start - 10:
                    ...
                    # Save to RAW_DIR
                    dest = RAW_DIR / f"orange_liens_{nowstamp()}.csv"
...
def download_pdf_orange(driver, instr_num: str) -> Optional[str]:
    """Download PDF for a Orange County lien via Acclaim portal."""
    import base64
    if not instr_num:
        return None
    safe     = re.sub(r"[^\w\-]", "_", instr_num)[:60]
    pdf_path = PDF_DIR / f"orange_{safe}.pdf"
    ...
    url = f"https://or.occompt.com/recorder/Details/GetDocumentByInstrumentNumber/{book_type}/{instr_num}"
    try:
        driver.get(url)
        time.sleep(3)
...
def main():
    parser = argparse.ArgumentParser(description="Orange County lien scraper")
    ...
    if records:
        snap = RAW_DIR / f"orange_liens_{nowstamp()}.json"
    ...
    if not args.no_pdf and records:
        ...
        try:
            ...
            for i, rec in enumerate(records):
                instr = rec.get("instrument_number", "")
                if instr:
                    pdf_path = download_pdf_orange(driver2, instr)
                    if pdf_path:
