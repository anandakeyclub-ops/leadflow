...
Usage:
  python -m app.workers.scrape_palm_beach_liens --days-back 90 --visible
  python -m app.workers.scrape_palm_beach_liens --days-back 90 --visible --no-db
  python -m app.workers.scrape_palm_beach_liens --days-back 90 --visible --no-pdf
"""
...
SOURCE_NAME = "palm_beach_irs_liens"
HOME_URL    = "https://erec.mypalmbeachclerk.com/home/index"

BASE_DIR  = Path(__file__).resolve().parents[2]
RAW_DIR   = BASE_DIR / "data" / "raw" / "palm_beach" / "irs_liens"
PDF_DIR   = RAW_DIR / "pdfs"
DEBUG_DIR = RAW_DIR / "debug"
for d in [RAW_DIR, PDF_DIR, DEBUG_DIR]:
    d.mkdir(parents=True, exist_ok=True)

# Palm Beach Landmark doc type IDs for tax liens
...
def make_driver(visible: bool = True) -> webdriver.Chrome:
    opts = Options()
    if not visible:
...
def setup_session(driver) -> bool:
    """Load home page and accept disclaimer."""
    driver.get(HOME_URL)
    time.sleep(5)
    try:
        ...
        print("  Disclaimer already accepted")
...
def search_doc_type(driver, doc_type: str, checkbox_id: str,
    ...
    # Open Document Type section via JS
    try:
        driver.execute_script("LaunchDisclaimerFromMenu('searchCriteriaDocuments');")
        time.sleep(2)
        print("  Opened Document Type section")
    except Exception as e:
        print(f"  Could not open section: {e}")

    # Step 1: Click "select" to open the document type modal
    try:
        driver.execute_script("ShowDocumentModal($('#documentTypeModal-DocumentType'));")
        time.sleep(1.5)
        print("  Opened doc type modal")
    except Exception as e:
        print(f"  Could not open modal: {e}")

    # Step 2: Clear existing selections then check our doc type
    try:
        driver.execute_script("ClearDocumentTypes('DocumentType');")
        time.sleep(0.3)
    except Exception:
        pass
    ...
    try:
        ...
        print(f"  Could not check {checkbox_id}: {e}")

    # Step 3: Click "Select" in modal to confirm → populates textarea
    try:
        driver.execute_script("UpdateDocumentTypeListFromModal('DocumentType');")
        time.sleep(1)
        print("  Confirmed modal selection")
    except Exception as e:
        print(f"  Modal confirm error: {e}")
        # Fallback: click the Select button directly
        try:
            btn = driver.find_element(By.XPATH,
                "//div[@id='documentTypeModal-DocumentType']"
                "//a[normalize-space(text())='Select']")
            driver.execute_script("arguments[0].click();", btn)
            time.sleep(1)
            print("  Clicked Select button in modal")
        except Exception:
            pass
    ...
    print()
    print("  " + "=" * 55)
    print(f"  ACTION REQUIRED for [{doc_type}]:")
    print("  1. Look at the browser window")
    print("  2. Solve the reCAPTCHA ('I am not a robot')")
    print("  3. Press Enter here when done")
    print("  " + "=" * 55)
    input("  Waiting... Press Enter after solving reCAPTCHA: ")
    print()
    ...
    if not submitted:
        try:
            ...
            submitted = True
            print("  Called SubmitSearch via JS")
        except Exception as e:
...
def parse_results(driver, doc_type: str,
    ...
    while True:
        ...
        rows_data = driver.execute_script("""
            var results = [];
            var base = 'https://erec.mypalmbeachclerk.com';
            var sr = document.getElementById('searchResults');
            ...
            while (i + RECORD_SIZE <= cells.length) {
                ...
                if (!debtor || debtor.length < 2 || /^\d+$/.test(debtor)) {
                    i += RECORD_SIZE;
                    continue;
                }
                var rd = {
    ...
    try:
        with conn.cursor() as cur:
            ...
            for rec in records:
                ...
                n_hash = f"pbc_irs::{rec.instrument_number}"
...
def main():
    ap = argparse.ArgumentParser(
        description="Palm Beach IRS federal tax lien scraper"
    )
    ...
    print(f"\n[Palm Beach IRS Liens] {start} → {end}")
    print("  NOTE: reCAPTCHA requires manual solving — browser must be visible")
    print()
    ...
    try:
        ...
        for doc_type, checkbox_id in IRS_DOC_TYPES:
            ...
            # Return to home between searches
            driver.get(HOME_URL)
            time.sleep(3)
    ...
    if all_records:
        snap = RAW_DIR / f"pbc_irs_{nowstamp()}.json"
        snap.write_text(
            json.dumps([{
